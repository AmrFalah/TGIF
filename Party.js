...
function getTenPer() {
  ...
  var table2 = document.getElementById("Party-max");
  ...
  for (var k = 0; k < data.results[0].members.length / 10; k++) {
    ...
    valTab1.innerHTML =
      data.results[0].members[k].total_votes -
      data.results[0].members[k].missed_votes;

    valTab2 = document.createElement("td");
    valTab2.innerHTML = data.results[0].members[k].votes_with_party_pct;
    rowTab1.append(a, valTab1, valTab2);
    table2.append(rowTab1);
  }
  for (
    var j = data.results[0].members.length * 0.1;
    j < data.results[0].members.length;
    j++
  ) {
    if (
      data.results[0].members[k].votes_with_party_pct ==
      data.results[0].members[k + 1].votes_with_party_pct
    ) {
      rowTab1 = document.createElement("tr");
      var a = document.createElement("a");
...
function getLastTenPer() {
  ...
  var table2 = document.getElementById("Party-min");
  ...
  for (var k = Math.round(data.results[0].members.length * 0.9); k > 0; k--) {
    if (
      ...
      valTab1 = document.createElement("td");
      valTab1.innerHTML =
        data.results[0].members[k].data.results[0].members[k].total_votes -
        data.results[0].members[k].missed_votes;
