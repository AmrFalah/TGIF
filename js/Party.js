...
function getMost(members) {
  ...
  for (var k = Math.round(members.length * 0.9); k > 0; k--) {
    if (
      members[k].votes_with_party_pct == members[k - 1].votes_with_party_pct
    ) {
      mostArray.push(members[k]);
    }
    break;
  ...
  createTable(mostArray, "Party-max");
}
function getLeast(members) {
  ...
  createTable(leastArray, "Party-min");
}
...
function createTable(array, id) {
  ...
  for (var k = 0; k < array.length; k++) {
    ...
    missedVotes.innerHTML = array[k].total_votes - array[k].missed_votes;

    missedVotesPer = document.createElement("td");
    missedVotesPer.innerHTML = array[k].votes_with_party_pct;
